...
async def process_callback_about(callback_query: types.CallbackQuery):
    await bot.send_message(callback_query.from_user.id, "Мы - талантливая команда разработчиков. Одной из наших недавних разработок является захватывающая игра, созданная специально для INNOPROG.\n"
